...
import (
	...
	"go.osspkg.com/goppy/v3/pkg/base64url"
	"go.osspkg.com/goppy/v3/pkg/byteops"

	"go.osspkg.com/goppy/v3/plugins/auth/token/algorithm"
...
func (v *_service) Create(payload json.Marshaler, audience string, lifetime time.Duration) (uuid.UUID, []byte, error) {
	...
	if _, err = w.Write(base64url.Encode(bh)); err != nil {
	...
	if _, err = w.Write(base64url.Encode(bp)); err != nil {
	...
	if _, err = w.Write(base64url.Encode(bs)); err != nil {
		return uuid.Nil, nil, fmt.Errorf("jwt: failed to write payload: %w", err)
	}
...
func (v *_service) Verify(token []byte) (*Header, []byte, error) {
	...
	header := &Header{}
	if err := json.Unmarshal(base64url.Decode(token[0:index[0]]), header); err != nil {
	...
	if err = alg.Verify(
		sig.KeyEntity,
		token[0:index[1]],
		base64url.Decode(token[index[1]+1:]),
	); err != nil {
		return nil, nil, fmt.Errorf("jwt: failed to verify signature: %w", err)
	}

	payload := base64url.Decode(token[index[0]+1 : index[1]])
	if len(payload) == 0 {
