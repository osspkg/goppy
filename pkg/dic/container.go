...
func New() *Container {
	return &Container{
		graph:   kahn.New(),
		brokers: make([]plugin.Broker, 0, 10),
		storage: newStorage(),
		status:  syncing.NewSwitch(),
	}
}

func (v *Container) BrokerRegister(args ...plugin.Broker) error {
	if v.status.IsOn() {
	...
	slices.SortFunc(v.brokers, func(a, b plugin.Broker) int {
