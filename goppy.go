...
func (v *_app) Resolve(arg any) error {
	if err := v.container.BreakPoint(arg); err != nil {
		return fmt.Errorf("register break point: %w", err)
	}
	if err := v.container.Register(arg); err != nil {
		return fmt.Errorf("register dependency: %w", err)
	}
	if err := v.container.Start(v.ctx); err != nil {
		return fmt.Errorf("start dependency: %w", err)
	}
	if err := v.container.Stop(); err != nil {
		return fmt.Errorf("stop dependency: %w", err)
	}
	return nil
}
...
func (v *_app) Run() {
	go events.OnStopSignal(v.ctx.Close)

	console.FatalIfErr(v.container.Register(
		func() xc.Context { return v.ctx },
	), "register base dependency")

	steps := appsteps.New(v.ctx.Context(),
		configInited, logInited, logDone, appExit,
	)

	wg := syncing.NewGroup(v.ctx.Context())
...

	{
		...
		v.console.RootCommand(console.NewCommand(func(setter console.CommandSetter) {
			...
			setter.ExecFunc(func(pid string) {
				...
				console.FatalIfErr(v.container.Start(v.ctx), "start dependency")
				<-v.ctx.Done()
